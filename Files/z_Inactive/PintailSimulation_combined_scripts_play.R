...
# start dependency on XLConnect — replaced with openxlsx (no Java) ####
...
# 
# new code: Helper to robustly detect "STOP" (handles NA/blank/spacing/case) ####
is_stop <- function(x) {
  if (length(x) == 0 || is.na(x)) return(FALSE)
  toupper(trimws(as.character(x))) == "STOP"
}

norm_txt <- function(x) toupper(trimws(as.character(x)))

## Find the number of node variables ####
if (ncol(DATA) < 3) stop("Sheet 2 has fewer than 3 columns; cannot locate node variables.")
row1_vals <- DATA[1, 3:ncol(DATA), drop = TRUE]
row1_norm <- norm_txt(row1_vals)

stop_pos <- match("STOP", row1_norm)
if (is.na(stop_pos)) {
  take_cols <- which(!is.na(row1_vals) & nzchar(trimws(as.character(row1_vals))))
} else {
  take_cols <- seq_len(stop_pos - 1)
}

varname_node <- as.list(as.character(row1_vals[take_cols]))
numvar_node <- length(varname_node)

## Find the number of path variables ####
start_row <- 1 + 3 * (num_nodes + 3)
if (nrow(DATA) < start_row) stop("Sheet 2 has fewer rows than expected; cannot locate path variables.")

step_size <- num_nodes + 3
rows_seq <- seq(from = start_row, to = nrow(DATA), by = step_size)
col1_vals <- DATA[rows_seq, 1, drop = TRUE]
col1_norm <- norm_txt(col1_vals)

stop_pos2 <- match("STOP", col1_norm)
if (is.na(stop_pos2)) {
  take_rows <- which(!is.na(col1_vals) & nzchar(trimws(as.character(col1_vals))))
} else {
  take_rows <- seq_len(stop_pos2 - 1)
}

varname_path <- list("p_ij", "s_ij")
if (length(take_rows) > 0) {
  extra <- as.list(as.character(col1_vals[take_rows]))
  varname_path <- c(varname_path, extra)
}
...
for (i in 1:NUMNET){
  ...
  ## new code ####
  # Read initial conditions robustly from sheet 1 (values in column B, starting at B2)
  init_raw <- openxlsx::read.xlsx(
    input_file_names[i],
    sheet = 1,
    colNames = FALSE,
    skipEmptyRows = FALSE,
  ...
  if (ncol(init_raw) < 2) {
    stop(paste0(
      "Sheet 1 has fewer than 2 columns. Expected node names in column A and initial counts in column B."
    ))
  }
  if (nrow(init_raw) < 2) {
    stop("Sheet 1 has no data rows (expecting data starting at B2).")
  }
  
  # Take column B from row 2 downward (B2:B…)
  colB <- init_raw[2:nrow(init_raw), 2]
  # Coerce to numeric (suppress warnings if cells are blank or text)
  colB_num <- suppressWarnings(as.numeric(colB))
  
  # Keep the contiguous non-NA block from the top
  if (all(is.na(colB_num))) {
    stop("No numeric initial conditions found in column B starting at B2.")
  }
  first_na <- which(is.na(colB_num))[1]
  if (is.na(first_na)) {
    init_vals <- colB_num
  } else {
    init_vals <- colB_num[seq_len(first_na - 1)]
  }
  
  # Validate length against num_nodes
  n <- length(init_vals)
  if (n != num_nodes) {
    stop(paste0(
      "\n Incorrect number of initial conditions given for species: ", networkname,
      "\n Found ", n, " contiguous numeric values in column B starting at B2, but expected ", num_nodes, ".",
      "\n Check that initial counts are in B2:B", 1 + num_nodes, " with no blanks in between."
    ))
  }
  
  # Store
  N_initial[[networkname]] <- init_vals
  nodes[[i]] <- n
  
  # Optional: also capture node names from column A (A2:A…)
  # names_init <- init_raw[2:(1 + n), 1]
  # names(N_initial[[networkname]]) <- as.character(names_init)
  
  ## Read in the remaining worksheets for each season ####
  ...
  for (k in 1:seasons){
    # READ IN THE DATA
    SR <- 2 # starting row for node data
    SC <- 3 # starting col for node data
    ECnode <- SC + numvar_node - 1
    ECpath <- SC + n - 1
    ...
    eps <- 0.000001
    if (any(x < 1 - eps & x != 0) | any(x > 1 + eps & x != 0)){
