...
# TEMP Call PintailSim_fn to calculate DS for given scenarioName ####
 quiet_f <- quietly(PintailSim_fn)
 DS <- quiet_f(scenarioName)

#DS <- PintailSim_fn(scenarioName)

# TEMP Ecosystem services per region  ----
...
# NEW Call PintailSim_fn to calculate DS for given scenarioName ####

compute_es_for_scenario <- function(scenarioName) {
  # Run the simulation quietly to get DS # DS <- PintailSim_fn(scenarioName)
  quiet_f <- purrr::quietly(PintailSim_fn)
  sim_res <- quiet_f(scenarioName)
  if (!is.null(sim_res$error)) stop(sprintf("PintailSim_fn('%s') failed: %s", scenarioName, sim_res$error$message))
  DS <- sim_res$result
  
  # Align Vi and Di to the canonical region order
  Vi <- wtp_out$total_WTP_M
  names(Vi) <- wtp_out$PintailRegion
  Vi <- Vi[regions]
  ...
  # Sanity checks
  if (any(!regions %in% names(Vi))) stop("Missing regions in Vi for scenarioName: ", scenarioName)
  if (length(Di) != length(regions)) stop("DS (Di) length mismatch for scenarioName: ", scenarioName)
  
  # Compute subsidies and related metrics
  Yi  <- sum(Vi) * Di - Vi      # net spatial subsidy per region
  
  # Tidy dataframe for this scenarioName
  tibble::tibble(
...
} # end function compute_es_for_scenario 

# Run all scenarios and bind results
results_long <- purrr::map_dfr(scenarios, compute_es_for_scenario)
...
p_subsidy <- ggplot2::ggplot(results_long, ggplot2::aes(x = scenarioName, y = subsidy, shape = region)) +
  ...
  ggplot2::labs(x = "scenarioName", y = "Region-specific subsidy (million 2016 USD)",
                shape = "Region",
                title = "Pintail migration subsidies by scenarioName and region") +
