...
for (sex in 1:numSex){
  ...
  ### Marc's code ####
  # We save CR & POP values for each season (CrCount / k)
...
for (sex in 1:numSex){
  ...
  if(sex == 1){
    ...
    EQ_POP_REGION <-
      unlist(N[[timestep]]$adult_female) +
      unlist(N[[timestep]]$juvenile_female)
  ...
  if(sex == 2){
    ...
    EQ_POP_REGION <-
      unlist(N[[timestep]]$adult_male) +
      unlist(N[[timestep]]$juvenile_male)
  ...
  HARVESTED_REGION <- EQ_POP_REGION * HARVEST_RATE
  
  HARVESTED_TOTAL <- sum(HARVESTED_REGION)
  
  # Save CR y POP values for each season (CrCount / k)
  for(season_k in 1:seasons) {
    formula_report <- data.frame(
      Scenario       = SIMNAME,
      Sex_Index      = sex,
